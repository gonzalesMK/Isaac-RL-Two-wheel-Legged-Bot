...
parser.add_argument(
    "--stack_frames", type=int, default=None, help="Number of frames to stack."
)
    ...
    if args_cli.plot == True:
        if flamingo_light:
            ...
            for i in range(2):
                plt.subplot(2, 2, i + 1)
                plt.plot(
                    [step[i] for step in joint_pos_list],
                    label=f"Joint Position {i + 1}",
                )
                plt.plot(
                    [step[i] for step in target_joint_pos_list],
                    label=f"Target Joint Position {i + 1}",
                    linestyle="--",
                )
                plt.title(
                    f"Joint Position {i + 1} and Target Joint Position",
                    fontsize=10,
                    pad=10,
                )
                plt.legend()
            ...
            for i in range(2):
                plt.subplot(2, 2, i + 3)
                plt.plot(
                    [step[i] for step in joint_velocity_obs_list],
                    label=f"Observed Joint Velocity {i + 1}",
                )
                plt.plot(
                    [step[i] for step in target_joint_velocity_list],
                    label=f"Target Joint Velocity {i + 1}",
                    linestyle="--",
                )
                plt.title(
                    f"Observed and Target Joint Velocity {i + 1}", fontsize=10, pad=10
                )
                plt.legend()
        else:
            plt.figure(figsize=(14, 16))
